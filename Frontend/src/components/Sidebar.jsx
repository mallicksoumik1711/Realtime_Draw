...
function Sidebar({ mobileSidebarOpen, setMobileSidebarOpen, menuItems }) {
    ...
    return (
        <aside
            className={`fixed lg:static z-50 h-full w-72 bg-white/98 backdrop-blur-2xl border-r border-gray-200/70 flex flex-col transition-transform duration-300 ${mobileSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
        >
            {/* Header */}
                ...
                {menuItems.map((item, i) => {
                    ...
                    return (
                        <button
                            ...
                            className={`w-full flex items-center gap-4 px-3 py-3 rounded-xl transition-all ${isActive ? "bg-teal-800 text-white shadow-lg" : "hover:bg-gray-100 text-gray-700"}`}
                        >
